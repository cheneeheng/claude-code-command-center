...
**Task:** per-project-plugin-toggler — release pppt-v0.9.4 carrying a fix that lives in `libs/claude-plugins`

**Context:** The description-truncation fix spans two members: the shared reader in `libs/claude-plugins` and its registered Node port in the toggler's `vscode-extension/extension.js`. Only the toggler is a releasable component (`docs/releasing.md` registers `pppt` alone), and `libs/claude-plugins` carries a `version` in `pyproject.toml` (`0.1.0`) but no `CHANGELOG.md` and no tag namespace. So a release that ships a library change had no recorded precedent: bump the library too, or let it ride under the component tag.

**Decision:** Ship the library fix under `pppt-v0.9.4` with no bump to `libs/claude-plugins`. Only the four toggler manifests moved 0.9.3 -> 0.9.4 (`pyproject.toml`, `vscode-extension/package.json`, and both lock files, regenerated with `uv lock` / `npm install --package-lock-only`). The library's user-visible behaviour change is recorded in the toggler's changelog entry, which names both readers, and in the two docs that describe the parser (`docs/shared-plugin-logic.md`, `libs/claude-plugins/README.md`). The bump commit rides the existing `fix/full-frontmatter-descriptions` PR rather than a separate `chore/release-` branch, matching Entry 67's release (the `pppt-v0.9.3` bump landed inside PR #45 and the tag pointed at that merge commit).

**Impact / Risk:** `libs/claude-plugins` version `0.1.0` now covers two different parser behaviours, so the version string cannot distinguish them. That is tolerable only while the library stays non-distributed and in-repo — its consumers (`claude-component-browser`, `per-project-plugin-toggler`) vendor it by path, not by version constraint. The moment it is published or pinned by version, it needs its own changelog and bump discipline, and this entry stops being a valid precedent. The second consumer, `claude-component-browser`, silently gains the fix with no release of its own; that is correct here, since it was showing the same truncated text.

**Outcome:** The four manifests were confirmed to read 0.9.4 and the changelog validated by the bundled semver checker (4 entries, newest-first, no duplicates). Both parsers were cross-checked against seven frontmatter shapes and agree on all of them, and the fix was measured against the installed plugin set: 226 members that the VSCode panel rendered at ~90 characters now render in full. `claude-plugins` is green on ruff, mypy, and pytest at its 100% coverage gate; the toggler's Windows smoke suite passes. Not verified: the extension was not launched, so the fix is proven at the parser level and not in a running panel. `tests/smoke.sh` fails on this machine before and after the change — it is the Linux/macOS path, and `tests/smoke.ps1` is the Windows one.
...
**Task:** New tool — daily commit and push of `.agents_workspace` repos across several projects

**Context:** The request was to model the tool on `scheduled-session-digests/daily-lessons` and log into the claude-meta `logs/` folder. Two things about the target folders had to be settled before any code: what to stage in each, and how to treat a repo sitting on a feature branch. The user's answer — each `.agents_workspace` is a git repo in its own right — conflicted with the only instance inspectable from this repo, whose `.agents_workspace/` has no `.git` and is tracked by the parent. The example the user then gave (`.../agent-skills/.agents_workspace`) resolved it: that one *is* standalone, on branch `agent-skills`, with remote `agents-workspace.git`. So the target folders are sibling branches of one shared notes remote, one branch per parent project, and this repo is simply not among them.

**Decision:** A new `tools/agents-workspace-sync` member rather than a fifth digest scheduler, since it shares no pipeline with the digests (no transcript scan, no cursor, no `claude --print`) and only the installer chrome would have been reused. It is a sibling of `git-sync`, not a caller: `git-sync.ps1` is hardcoded to the single `$C4_CLAUDE_META_DIR` repo. Two invariants carry the design. First, every configured path must be the top level of its own repo, tested with `git rev-parse --show-prefix` being empty; a path that fails is logged and skipped. Second, each repo is committed and pushed on the branch already checked out — no `checkout`, `merge`, or `branch` — which is the deliberate inversion of `git-sync`'s merge-to-default, correct there because it owns `claude-meta` outright and wrong here where the branches are the organising axis.

**Impact / Risk:** The root-repo check is load-bearing, not defensive: without it `git -C <nested> add -A` walks up to the outer `.git` and stages the entire parent project, so a single misconfigured entry would commit and push unrelated work-in-progress nightly across every repo in the list. Two implementations of that check were written and discarded during the build. Comparing `--show-toplevel` against the configured path textually fails on Git Bash, where git prints `C:/x/y` and `pwd` prints `/c/x/y` — it rejected every repo; case, symlinks, and 8.3 short names break it too. `--show-prefix` needs no normalisation and works on both platforms. Separately, `Sync-Repo` originally returned `$true`/`$false` while `Log` wrote to the success stream, so the returned value arrived mixed into the log lines and `if (-not (Sync-Repo $r))` tested a non-empty array — always truthy, silently swallowing every failure and hiding the log output. It now reports through `$script:Failed`. Both traps are recorded as invariants in the member's `CLAUDE.md`.

**Outcome:** Verified against throwaway repos with a bare local remote: first push sets upstream, a clean second run commits and pushes nothing, a new change commits and pushes one commit, and the parent-path config form resolves to the nested repo — in both the PowerShell and Bash workers, with the branch never moving. The guard was verified against a real nested case, this repo's own `.agents_workspace`, and correctly refused it while leaving the tree unstaged (`git status` clean afterwards). Mixed batches exit `2` with the healthy repos still synced. Not verified: the scheduled task and crontab entry have not been registered or fired, no push to a real remote was made, and the Linux installer was not run on Linux. An initial push test failed with `Filename too long` — the scratchpad path exceeding Windows `MAX_PATH`, not a tool defect; it passed on a short path.
