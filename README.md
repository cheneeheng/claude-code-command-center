...
| Tool | What it does |
|------|--------------|
| [`statusline-cost-dashboard`](tools/statusline-cost-dashboard/) | Terminal statusline hook plus a live token/cost dashboard for Claude Code sessions. |
...
> The first four tools were originally one `claude-automation` suite; see
> [`docs/automation-suite.md`](docs/automation-suite.md) for the combined overview.

### `libs/` — shared libraries

Empty for now. Planned: a dependency-light parser for `~/.claude/**/*.jsonl` transcripts,
extracted from the dashboard / plan-runner / scheduler that all read those logs today.
