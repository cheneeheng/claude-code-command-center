...
#
# Interactive-session variant of daily-lessons-trigger.sh. Does everything the
...
JOBS_DIR="$META_DIR/.claude/scheduler-jobs/daily-lessons"
...
# Reset staging dir and write an empty manifest up front so the skill always has
...
if [[ ${#SCAN_DIRS[@]} -eq 0 ]]; then
    log "[daily-lessons-prepare] No projects directory found (checked $PROJECTS_DIR and $DEVCONTAINER_PROJECTS_DIR) - nothing to prepare."
...
if [[ ${#RECENT_FILES[@]} -eq 0 ]]; then
    log "[daily-lessons-prepare] No chat histories updated since $CUTOFF_LABEL - nothing to prepare."
    echo "MANIFEST=$MANIFEST"
    echo "JOBS=0"
    exit 0
fi

log "[daily-lessons-prepare] Found ${#RECENT_FILES[@]} chat(s) modified since $CUTOFF_LABEL."

# Accumulate manifest entries as compact JSON objects, one per line.
JOBS_TMP=$(mktemp)
TRANSCRIPT_TMP=""
cleanup() {
...
for file in "${RECENT_FILES[@]}"; do
    ...
    log "[daily-lessons-prepare] Staging: $UUID ($CHAT_DATE)..."
    ...
    if [[ $USER_TURNS -lt $MIN_USER_TURNS ]] || [[ $TRANSCRIPT_LEN -lt $MIN_TRANSCRIPT_CHARS ]]; then
        log "[daily-lessons-prepare] Skipping $UUID - too short ($USER_TURNS turn(s), $TRANSCRIPT_LEN chars)."
        rm -f "$TRANSCRIPT_TMP"; TRANSCRIPT_TMP=""
        continue
    fi
...
log "[daily-lessons-prepare] Staged $PREPARED job(s) -> $MANIFEST"
echo "MANIFEST=$MANIFEST"
echo "JOBS=$PREPARED"
